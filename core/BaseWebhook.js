...
class BaseWebhook {
  ...
  async handle(req, res) {
    try {
      ...
      // Handle A1Zap webhook connection tests without invoking AI or A1Zap APIs
      if (event === 'webhook.test' || req.body?.metadata?.test === true) {
        console.log('🧪 Webhook test event detected - responding without processing');
        return res.json({
          success: true,
          event: 'webhook.test',
          agent: this.agent.name,
          message: 'Webhook is reachable and configured.'
        });
      }
      ...
      // Clean user message from repetitions (e.g., "text: text: text:")
      const cleanedUserMessage = this.cleanRepeatedUserInput(data.userMessage);
      data.userMessage = cleanedUserMessage;
      
      console.log(`Processing request for ${this.agent.name} from chat ${data.chatId}`);
      console.log(`User message: "${cleanedUserMessage}"`);
  ...
  /**
   * Clean up repeated user input (e.g., "text: text: text:")
   * @param {string} text - User input to clean
   * @returns {string} Cleaned text
   */
  cleanRepeatedUserInput(text) {
    if (!text || typeof text !== 'string') return text;
    
    // Detect if input is repeated (same phrase multiple times with colon separator)
    const colonSplit = text.split(':').map(s => s.trim());
    if (colonSplit.length > 5) {
      // Check if most parts are identical
      const firstPart = colonSplit[0].toLowerCase();
      const identicalCount = colonSplit.filter(p => p.toLowerCase() === firstPart).length;
      
      if (identicalCount > colonSplit.length * 0.7) {
        // More than 70% identical - this is repetition
        console.log(`🧹 Cleaned repeated user input (${colonSplit.length} repetitions)`);
        return firstPart;
      }
    }
  ...
  /**
   * Clean up repeated agent name prefixes that A1Zap sometimes adds
   * Example: "Georgie Gem Miner: Georgie Gem Miner: Georgie Gem Miner: Hello" → "Hello"
   * @param {string} text - Text to clean
   * @returns {string} Cleaned text
   */
  cleanRepeatedAgentName(text) {
    if (!text || typeof text !== 'string') return text;
    
    const agentName = this.agent.name;
    if (!agentName) return text;
    
    // Try multiple variations of the agent name that A1Zap might use
    const nameVariations = [
      'Georgie Gem Miner',  // A1Zap uses this version (without "the")
      agentName,  // Full name: "Georgie the Gem Miner"
      agentName.replace(/\s+the\s+/gi, ' '),  // Without "the"
      agentName.split(' ')[0]  // First name only: "Georgie"
    ];
    
    let cleaned = text;
    let foundMatch = false;
    
    // Try to clean with each name variation
    for (const name of nameVariations) {
      const escapedName = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      // Match one or more repetitions at the start, with or without spaces around colons
      const pattern = new RegExp(`^(${escapedName}\\s*:\\s*)+`, 'gi');
      const testCleaned = cleaned.replace(pattern, '').trim();
      
      // If this variation removed something, use it
      if (testCleaned !== cleaned && testCleaned.length > 0) {
        console.log(`🧹 Cleaned repeated agent name (matched: "${name}")`);
        console.log(`   Removed ${Math.floor((cleaned.length - testCleaned.length) / name.length)} repetitions`);
        cleaned = testCleaned;
        foundMatch = true;
        break;
      }
    ...
    // If we still have the pattern, do a more aggressive cleanup
    if (!foundMatch && (cleaned.includes('Georgie') && cleaned.indexOf(':') < 100)) {
      // Remove everything before the first non-name text
      const aggressivePattern = /^(Georgie[^:]*:\s*)+/gi;
      const aggressiveCleaned = cleaned.replace(aggressivePattern, '').trim();
      if (aggressiveCleaned !== cleaned && aggressiveCleaned.length > 0) {
  ...
  async sendResponse(chatId, result) {
    ...
    // Send text response
    if (result.response) {
      // Clean up any repeated agent name prefixes before sending
      const cleanedResponse = this.cleanRepeatedAgentName(result.response);
  ...
  async handleError(error, req, res) {
    ...
    try {
      ...
      if (chatId && !webhookHelpers.isTestChat(chatId)) {
        await this.client.sendMessage(chatId, this.getErrorMessage(error));
      }
    } catch (sendError) {
  ...
  getErrorMessage(error) {
    if (error && error.code === 'GEMINI_API_KEY_INVALID') {
      return "I’m having trouble reaching my ideas engine right now (API key issue). Please try again in a moment while I refresh my connection.";
    }
